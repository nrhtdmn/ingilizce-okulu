 ...
 *     smtp.from="İngilizce Okulu <gonderici@gmail.com>"
 *
...
exports.onAnnouncementForEmail = functions.firestore
  ...
  .onWrite(async (change, context) => {
    ...
    const subject = latest.forUsername
      ? "Ingilizce Okulu — Size ozel bildirim"
      : "Ingilizce Okulu — Yeni duyuru";
  ...
  .onWrite(async (change, context) => {
    ...
    for (const asgn of newAssignments) {
      ...
      for (const stu of list) {
        ...
        if (!to) continue;
        const subj = "Ingilizce Okulu — Yeni odev";
        const body =
    ...
    for (const key of Object.keys(afterSub)) {
      ...
      const subj = "Ingilizce Okulu — Ogretmen degerlendirmesi";
      const body =
    ...
    for (const key of newSubmissionKeys) {
      ...
      const subj = "Ingilizce Okulu — Odev teslim edildi";
      const body =
    ...
    for (const tkey of Object.keys(afterThreads)) {
      ...
      for (let i = arrBefore.length; i < arrAfter.length; i++) {
        ...
        if (!to) continue;
        const subj = "Ingilizce Okulu — Yeni mesaj";
